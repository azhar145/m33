...
##    p = plt.plot(dfp[:i].index, dfp[:i].values,'o-', lw = 2,color='b') #note it only returns the dataset, up to the point i
    p2 = plt.plot(dfp[:i].index, dfp['close'][:i].values,'-', lw = 1,color='r',label='close') #note it only returns the dataset, up to the point i
    p3 = plt.plot(dfp[:i].index, dfp['vwap'][:i].values,'o', lw = 1,color='b',label='vwap')
    p4 = plt.plot(dfp[:i].index, dfp['EMA_50'][:i].values,'--', lw = 0.8,color='grey',label='EMA_50')
    p4a = plt.plot(dfp[:i].index, dfp['EMA_100'][:i].values,'--', lw = 1.2,color='grey',label='EMA_100')
    p4a = plt.plot(dfp[:i].index, dfp['EMA_200'][:i].values,'--', lw = 1.4,color='grey',label='EMA_200')
    p5 = plt.plot(dfp[:i].index, dfp['EMA_10'][:i].values,'-', lw = .5,color='g',label='EMA_10')
    p6 = plt.plot(dfp[:i].index, dfp['EMA_21'][:i].values,'-', lw = .4,color='y',label='EMA_21')
    p7 = plt.plot(dfp[:i].index, dfp['SAR'][:i].values,'-', lw = .4,color='cyan',label='SAR')
